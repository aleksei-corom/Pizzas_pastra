...
from config import CURRENCY_SYMBOL, ORDER_STATUS, ORDER_TYPES
...
class OrderDetailDialog(QDialog):
    ...
    def __init__(self, orden, items, parent=None):
        ...
        tipo_text = ORDER_TYPES.get(orden.tipo, orden.tipo)
        ...
        for item in items:
            ...
            price = QLabel(f"{CURRENCY_SYMBOL}{item.subtotal:.2f}")
        ...
        for lbl, val in [("Subtotal", orden.subtotal), ("Impuesto", orden.impuesto)]:
            ...
            row.addWidget(l, 1)
            v = QLabel(f"{CURRENCY_SYMBOL}{val:.2f}")
            row.addWidget(v)
            layout.addLayout(row)
        ...
        t_val = QLabel(f"{CURRENCY_SYMBOL}{orden.total:.2f}")
...
class OrdenesView(QWidget):
    ...
    def cargar_datos(self):
        ...
        for i, row_data in enumerate(rows):
            ...
            self._table.setItem(i, 4, QTableWidgetItem(f"{CURRENCY_SYMBOL}{orden.total:.2f}"))
            ...
            if orden.estado in next_states:
                ...
                btn_next.setToolTip(f"Cambiar a: {ORDER_STATUS.get(next_st, '')}")
    ...
    def _ver_detalle(self, orden):
        items = self.db.get_orden_items(orden.id)
